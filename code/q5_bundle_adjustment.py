...
Q5.1: RANSAC method.
    ...
    (1) You can use the calc_epi_error from q1 with threshold to calcualte inliers. Tune the threshold based on 
        the results/expected number of inliners. You can also define your own metric. -Done
    (2) Use the seven point alogrithm to estimate the fundamental matrix as done in q1 - I used 8
...
def ransacF(pts1, pts2, M, nIters=1000, tol=10):
    ...
    print((prev_inliers/N)*100) #to be 75%s
...
if __name__ == "__main__":
    ...
    F_e = eightpoint(noisy_pts1, noisy_pts2, M=np.max([*im1.shape, *im2.shape]))
    print("Eight Point (noisy points")
    print(F_e)
    ...
    print("F reestimated from RANSAC")
    print(F)
